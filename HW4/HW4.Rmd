...
s <- Y2[1,1:5]
s <- t(data.frame(s))
apply(format(s), 1, paste, collapse="")
paste(s[1,], collapse = "")

# no se, we have to produce columns like "A G D C A D S D A A" or whatever and thed we can use the 
# other function
Geno2 <- matrix(nrow = length(Y2[1,]))
for(i in 1:length(Y2[1,])){
  print(i)
  rbind(Geno2, cbind(substr(Y2[,i],1,1), substr(Y2[,i],2,2)))
}
