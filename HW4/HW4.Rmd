...
is.hetero <- function(x) {
  return(substr(x, 1, 1) != substr(x, 2, 2))
}

num.hetero <- function(x) {
  return(sum(is.hetero(x[!is.na(x)])))
}
...
AmbiGeno <- cbind(substr(Y[,1],1,1),substr(Y[,1],2,2))
for(i in 2:ncol(Y)) {
  AmbiGeno <- cbind(AmbiGeno,substr(Y[,i],1,1),substr(Y[,i],2,2))
}
