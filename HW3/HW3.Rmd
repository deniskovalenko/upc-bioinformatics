...
```
Most common is GG gaplotype.




## Task 7 (2p)
### Compute 4 LD statistics for all the marker pairs in this data base (D, D', Ï2 and R2). Make
#a scatterplot matrix of these. Is there an exact linear relationship between Ï2 and R2 ? Why (not) so?
...
## Task 8. (2p) Compute a distance matrix with the distance in base pairs between all possible pairs of SNPs. Make a plot of the R2 statistics against the distance between the markers. Comment on your results
...
```
# Results are incosistent - with D' we can see that D's are uniformly distributed and are close to 1 for most of SNP pairs, while with R^2 we see several clusters of closely positioned SNPS to have high R^2

## Task 10. (2p) 

#### Simulate 45 independent SNPs under the assumption of Hardy-Weinberg equilibrium. Simulate as many SNPs as #### you have in your database, and take care to match each SNP in your database
#### with a simulated SNP that has the same sample size and allele frequency. Make two LD heatmaps
#### of the simulated SNPs, one using R^2 and one using D'. Compare these to the LD heatmap of the
#### ABO region. What do you observe? State your conclusions
...
for(i in 1:nrow(simulated)){
  ...
  sim_df <- cbind(sim_df, data.frame(genotype(generated_row)))
}

rgb.palette <- colorRampPalette(rev(c("blue", "orange", "red")), space = "rgb")
LDheatmap(simulated, LDmeasure="D'", add.map=TRUE, title="D' LD heatmap",color=rgb.palette(18))

as.genotype(simulated[1,])

```

## (1p) Do you think there is strong or weak LD for the ABO region you just studied? Explain your opinion